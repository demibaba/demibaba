...
const HeartIcon = () => <Ionicons name="heart" size={16} color="#FF6B6B" />;
const PhoneIcon = () => <Ionicons name="call" size={16} color="#FFFFFF" />;
...
export default function ReportDetailScreen() {
  ...
  // EmotionData에 emotion 속성 추가
  const getEmotionFromScore = (overall: number): string => {
    if (overall >= 8) return '😊';
    if (overall >= 6) return '😌';
    if (overall >= 4) return '😐';
    if (overall >= 2) return '😔';
  ...
  const renderPeriodChart = () => {
    ...
    // 차트 데이터 준비 (사용되지 않음 - View 컴포넌트로 대체됨)
    ...
    return (
      <View style={styles.periodChartContainer}>
        ...
        {/* LineChart를 간단한 View로 대체 */}
        <View style={styles.simpleChartContainer}>
          <DefaultText style={styles.simpleChartTitle}>기분 변화 그래프</DefaultText>
          <View style={styles.simpleChart}>
            {periodEmotionData.map((entry, index) => (
              <View key={index} style={styles.simpleChartBar}>
                <View 
                  style={[
                ...
                />
                <DefaultText style={styles.simpleChartLabel}>
                  {new Date(entry.date).getDate()}
                </DefaultText>
              </View>
            ))}
                    ...
                    if (parsedEmotions !== null && parsedEmotions.length < 7) {
                      ...
                      if (lastEmotion) {
                        ...
                        while (parsedEmotions !== null && parsedEmotions.length < 7) {
                          const newEmotion = {
                            ...lastEmotion,
                            day: days[parsedEmotions.length] || "일요일",
    ...
    try {
      ...
      if (!querySnapshot.empty && querySnapshot.docs.length > 0) {
        ...
        if (firstDoc && firstDoc.exists()) {
          const spouseData = firstDoc.data() as SpouseInfo;
      ...
      <View style={styles.chartCard}>
        ...
        <View style={styles.lineChartContainer}>
          ...
          <View style={styles.simpleChartContainer}>
            <DefaultText style={styles.simpleChartTitle}>기분 변화 그래프</DefaultText>
            <View style={styles.simpleChart}>
              {emotionScores.map((entry, index) => (
                <View key={index} style={styles.simpleChartBar}>
                  <View 
                    style={[
                      styles.simpleChartBarFill, 
                      { 
                        height: `${(entry.overall / 10) * 100}%`,
                        backgroundColor: getEmotionColor(getEmotionFromScore(entry.overall))
                      }
                    ]} 
                  />
                  <DefaultText style={styles.simpleChartLabel}>
