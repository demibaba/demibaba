...
export default function ReportsListScreen() {
  ...
  const handleCoupleAnalysis = async () => {
    try {
      ...
      console.log("부부 일기 수집 및 분석 시작...");
      
      // 부부 일기 수집
      const { myDiaries, spouseDiaries } = await collectCoupleDiaries(userId, spouseId);
      
      if (myDiaries.length === 0 && spouseDiaries.length === 0) {
        console.log("분석할 일기가 없습니다.");
        return;
      }

      // 통합 일기 텍스트 생성
      const coupleDiaryText = createCoupleDiaryText(myDiaries, spouseDiaries);
      
      console.log("부부 일기 분석 시작...");
      
      // Claude API로 부부 분석 레포트 생성
      const aiResponse = await generateClaudeReport(coupleDiaryText, spouseDiaries);
      
      console.log("부부 분석 레포트 생성 완료");
      
      // 레포트 저장 로직은 기존과 동일하게 구현
      // 여기서는 성공 메시지만 표시
      console.log("부부 분석이 완료되었습니다.");
      
    } catch (error) {
      console.error("부부 분석 오류:", error);
    }
  };
